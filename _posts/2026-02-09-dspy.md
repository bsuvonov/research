...
DSPy is a framework for building language-model applications with a programming mindset instead of relying on manual prompt trial-and-error. The language models are provided with intuitions, such as "document -> summary", which tells to return a summary for a given document. DSPy framework takes care of the prompt generation. For further optional optimizing the prompts, there are optional evaluation and optimization stages.

Evaluation requires three foundations: representative data, a clear quality metric, and a stable baseline pipeline. Logically, evaluation runs the same pipeline on held-out examples, scores outputs with the metric, and surfaces both aggregate performance and failure patterns.

Optimization requires a reliable evaluation signal plus an optimizer that can search for better program variants. Logically, optimization proposes candidate variants (for example, better instructions and demonstrations), evaluates them on validation data, keeps the strongest version, and repeats until gains flatten.

This creates a closed loop: define behavior, evaluate consistently, optimize against measured errors, then re-evaluate to confirm real improvement.

This note walks through that flow from core concepts to practical evaluation and optimization.
...
DSPy follows the usual ML split: `trainset`, `devset`, and `testset`.

- `trainset`: used by optimizers to improve prompts/program behavior.
- `devset`: development set (validation-like set) used to compare program variants and track progress with a metric.
- `testset`: held out for final reporting, not for iterative tuning.

Each datapoint is usually represented as `dspy.Example`, and can contain inputs, intermediate labels, and final labels. In practice, you can start with only inputs and add labels as your pipeline matures. Also, module outputs are `dspy.Prediction`, which is a specialized subtype of `Example`.

```py
qa = dspy.Example(question="What is 2+2?", answer="4")
print(qa.question)  # dot access works
```

------------------------------------

In DSPy, inputs are marked explicitly with `.with_inputs(...)`. Anything not marked as input is treated as non-input metadata/labels.

```py
qa = dspy.Example(question="What is 2+2?", answer="4").with_inputs("question")

print(qa.inputs())  # Example({'question': 'What is 2+2?'})
print(qa.labels())  # Example({'answer': '4'})
```

This input-key marking is important because evaluation/optimization code often does:
...
More on data handling: https://dspy.ai/learn/evaluation/data/

### Metrics

A DSPy metric is just a Python function, typically `metric(example, pred, trace=None)`, returning `bool`, `int`, or `float`.

For simple tasks (classification, short QA), start with direct metrics like exact match / accuracy. DSPy includes useful built-ins:
...
For long-form outputs, metric logic is usually richer and may check multiple properties. In practice, many teams return a graded score during evaluation, but a strict pass/fail when DSPy is bootstrapping examples:

```py
def metric(example, pred, trace=None):
    score = 0.0  # your grader result, usually a float

    # During optimization/bootstrapping, DSPy may pass a trace.
...
Evaluation can be done in a manual loop, or with `dspy.evaluate.Evaluate` for progress display and multi-threaded runs.

For advanced setups, the metric itself can be a DSPy program (or LM judge) and optimized too. In those cases, `trace` can also be used to inspect whether intermediate reasoning/tool use followed your constraints.

More on metrics: https://dspy.ai/learn/evaluation/metrics/
...
After evaluation, the next step is optimization. In DSPy, an optimizer searches for better program behavior by tuning instructions, demos (few-shot examples), and in some setups even LM weights.

Conceptually, optimization loops over candidate program variants, scores them with your metric on validation data, and keeps the best-performing version.

------------------------------------

In code, the optimizer takes a `student` program plus data/metric context, then returns a compiled (optimized) program:
...
If `valset` is omitted, many optimizers can split from `trainset` automatically.

------------------------------------

Data split guidance from DSPy docs:

- If you have fewer than ~200 examples, use a smaller train split and larger validation split (for example, 20/80).
- If you have ~200+ examples, a typical split is 80/20.
- For GEPA specifically, use roughly 80/20 train/validation.

If you do not have data yet, start by manually annotating around 30 examples, run optimization, inspect failures, and keep adding examples that cover failure modes plus random samples.

More on optimization overview: https://dspy.ai/learn/optimization/overview/



### Optimizers

There are two main axes to optimize:

1. **Instructions** (prompt-level behavior)
2. **Few-shot demonstrations** (which examples are shown to the LM)

Some optimizers also support weight-level adaptation through fine-tuning.

------------------------------------

How the optimization stage is done, logically:

1. Start from a baseline DSPy program and a metric.
2. Optimizer generates candidate program variants (different instructions/demos, depending on optimizer).
3. Each candidate is run on validation examples and scored with the same metric.
4. The optimizer keeps stronger candidates and discards weaker ones.
5. Search continues for several rounds (or until budget/early-stop), then returns the best compiled program.

In short: optimization is metric-guided search over program variants, not a one-shot prompt rewrite.

```py
# Baseline program
program = dspy.ChainOfThought("question -> answer")

# Pick an optimizer and run compile
...
# Compare before/after on the same devset
before = dspy.evaluate.Evaluate(devset=devset, metric=metric)(program)
after = dspy.evaluate.Evaluate(devset=devset, metric=metric)(optimized_program)
```

What this means in practice:

- Better metrics lead to better optimization decisions.
- Better validation data coverage leads to more robust compiled programs.
- Optimization can overfit if the validation set is too small or too narrow, so keep a separate final `testset`.

------------------------------------

Main optimizer families and what they optimize:
...
Automatic few-shot optimizers:

1. **`LabeledFewShot`**: selects labeled examples directly as demos.
2. **`BootstrapFewShot`**: uses a teacher program to generate demos and keeps demos that pass metric checks.
3. **`BootstrapFewShotWithRandomSearch`**: runs multiple randomized bootstrap candidates and picks the strongest compiled program.
4. **`KNNFewShot`**: chooses nearest training examples per input, then uses them in the bootstrap flow.

------------------------------------

Automatic instruction optimizers:

1. **`COPRO`**: improves instructions via coordinate-ascent style search.
2. **`MIPROv2`**: jointly optimizes instructions + demos with Bayesian optimization over candidates.
3. **`SIMBA`**: uses stochastic mini-batches to find unstable/hard cases, then generates self-reflective improvement rules or adds strong demos.
4. **`GEPA`**: reflective prompt evolution; inspects trajectories, proposes improved prompts, and can use textual feedback (not just scalar scores) to guide evolution.

GEPA-specific note: GEPA is especially useful when metric scores alone are too weak, and short textual feedback can explain why outputs failed.

------------------------------------

Other optimizers:

1. **`BootstrapFinetune`**: distills prompt behavior into model weight updates (when finetuning is supported).
2. **`Ensemble`**: combines multiple compiled programs into one inference strategy.
3. **`BetterTogether`**: composes optimization strategies (for example, stronger prompt optimization followed by finetuning).

------------------------------------

More on optimizers: https://dspy.ai/learn/optimization/optimizers/
