...
DSPy renamed this system from **teleprompters** to **optimizers**, but you may still see both terms in older code/docs.
...
Common optimizer choices and when to use them:

1. **`BootstrapFewShot`**: quick baseline for few-shot pipelines; often a good first optimizer.
2. **`BootstrapFewShotWithRandomSearch`**: same idea, but explores more candidates with random search.
3. **`MIPROv2`**: strong default for jointly optimizing instructions + demos, especially for harder tasks.
4. **`COPRO`**: useful for zero-shot-ish scenarios where instruction optimization is the primary lever.
5. **`BootstrapFinetune`**: fine-tunes model weights per module (provider support required).
6. **`BetterTogether`**: combines multiple optimization strategies and can outperform single-optimizer setups.

Practical starter path: begin with `BootstrapFewShot` for a baseline, then move to `MIPROv2` when you need stronger quality.
